...
export const selectCourse = async () => {
  ...
  const courses = await inquirer
    .prompt({
      type: "autocomplete",
      name: "selectedCourse",
      message:
        "Bitte wählen Sie einen Kurs oder geben Sie den Kursname ein, drücken Sie die Taste <ESC>, um zurück zum Hauptmenü zu gehen",
...
export const bookSportsCard = async () => {
  ...
  if (bookingMenu) {
    console.log(chalk.green("Buchungsmenü ist gefunden!"));
  } else {
    console.error(chalk.red("Buchungsmenü nicht gefunden."));
    await browser.close();
    process.exit(1);
  }
  ...
  for (const row of rows) {
    try {
      const button = await row.$(
        'td:last-child input[type="submit"].bs_btn_buchen',
      );
      if (button) {
        const buttonName = await button.getAttribute("name");
        if (isDebugMode) console.log(`Buchung verfügbar: ${buttonName}`);
...
const selectCourseDay = async (courseName) => {
  ...
  if (container) {
    ...
    if (anchors.length > 0) {
      ...
      for (const anchor of anchors) {
        ...
        if (isDebugMode)
          console.debug(`Vergleich von ${anchorText} mit ${courseName}...`);
  ...
  const { tableData, courseIDs } = await page.evaluate(() => {
    ...
    rows.forEach((row) => {
      if (!row.querySelector(".bs_sdet")) return;
      let tableObject = {
        name: "",
        place: "",
        time: "",
      ...
      const detSpan = row.querySelector(".bs_sdet > span");
      if (detSpan) tableObject.name = detSpan.textContent.trim();

      const tzRows = row.querySelectorAll(".bs_tzotable tr");
      const places = [];
      tzRows.forEach((tzRow, i) => {
        if (i === 0) {
          tableObject.day =
            tzRow.querySelector(".bs_stag")?.textContent.trim() ?? "";
          tableObject.time =
            tzRow.querySelector(".bs_szeit")?.textContent.trim() ?? "";
        }
        const loc = tzRow.querySelector(".bs_sort")?.textContent.trim();
        if (loc) places.push(loc);
      });
      tableObject.place = places.join(" / ");

      const button = row.querySelector('.bs_sbuch input[type="submit"]');
      if (button) {
        tableObject.status = button.value.trim();
        tableObject.id = button.name;
        if (button.classList.contains("bs_btn_buchen")) {
          localIDs.push(button.name);
        } else if (
          button.classList.contains("bs_btn_ausgebucht") ||
          button.classList.contains("bs_btn_warteliste")
        ) {
          tableObject.disabled = "Dieser Kurs ist ausgebucht!";
          localIDs.push(button.name);
        }
      } else {
        const statusCell = row.querySelector(".bs_sbuch");
        tableObject.disabled = statusCell?.textContent.trim() ?? "";
        localIDs.push("x");
      }
  ...
  const courseInfoCombined = tableData.map((course, _) => {
    ...
    const coloredCourseInfo = isDisabled
      ? chalk.red(courseInfo)
      : chalk.green(courseInfo);
    return {
  ...
  const germanText = await page.$eval(".bs_kursbeschreibung .bslang_de", (el) =>
    el.innerText.trim(),
  );
  console.log(boxen(germanText));

  const availableCourses = await inquirer
    .prompt({
      type: "list",
  ...
  if (isDebugMode)
    console.debug(`Die Taste mit dem ID ${courseID.id} wurde geclickt...`);
  ...
  try {
    ...
    console.error(
      "Fehler beim Clicken auf der Taste oder Wartung auf das Ziel:",
      error,
    );
    page = null;
  }
  ...
  if (dateSelector.length > 0) {
    ...
    const bookingButton = await page.$(
      'input[type="submit"][class="inlbutton buchen"][value="buchen"]',
    );
    if (bookingButton) {
      await Promise.all([
    ...
    console.error(
      "Es ist zurzeit keine Buchung zu diesem Angebot verfügbar.Gehe zurück ins Menü...",
    );
...
const fillCredentials = async (page, courseName, courseID, date) => {
  try {
    console.log(
      "Die Textfelder wird ausgefüllt, bitte haben Sie etwas Geduld...",
    );

    await page
      .locator(`input[name="sex"][value="${process.env.GESCHLECHT}"]`)
      .check();
    ...
    if (
      (await matriculationField.isVisible()) &&
      (await matriculationField.isEnabled())
    ) {
      await matriculationField.click();
      await matriculationField.fill(process.env.MATRIKEL_NR);
      if (isDebugMode)
        console.debug(`Matrikelnummer: ${process.env.MATRIKEL_NR}`);
    }
    ...
    if ((await phoneField.isVisible()) && (await phoneField.isEnabled())) {
      ...
      if (isDebugMode)
        console.debug(`Dienstleistungsnummer: ${process.env.DIENSTL_NR}`);
    }
    ...
    if (isDebugMode)
      console.log(
        "Submit-Button wurde geclickt, die Informationen wird geprüft...",
      );
    ...
    try {
      ...
      if (isDebugMode)
        console.log("Alles gut. Keine Validierungsfehler verfügbar.");
    }

    const warnings = await page.evaluate(() => {
      return Array.from(document.querySelectorAll(".bs_form_row.warn")).map(
        (row) => {
    ...
    if (warnings.length > 0) {
      console.log(
        "Das/die folgende(n) Feld(er) ist/sind entweder falsch ausgefüllt oder leer",
      );
      warnings.forEach((w) => console.log(" -", w));
      process.exit(0);
    } else {
      console.log("Keine Warnungen, geht weiter.");
    }
    await page.waitForLoadState("networkidle");
    await page.waitForSelector(
      'input[type="submit"][value="verbindlich buchen"], input[type="submit"][value="kostenpflichtig buchen"]',
      { timeout: 10000 },
    );

    const verbindlichBtn = page.locator(
      'input[type="submit"][value="verbindlich buchen"]',
    );
    const kostenpflichtigBtn = page.locator(
      'input[type="submit"][value="kostenpflichtig buchen"]',
    );
    ...
    await page.waitForLoadState("networkidle");
    const alreadyBooked = await page
      .locator('form[name="bsform"] .bs_meldung')
      .isVisible();
    if (alreadyBooked) {
      const bookingMessage = await page
        .locator('form[name="bsform"] .bs_meldung')
        .innerText();
    ...
    if (bookingCompleted) {
      ...
      console.log(
        `Buchungsfoto kann in \"screenshots\"-Ordner gefunden werden`,
      );
